...
public class GamerGirlAttackSymbol {
	...
	public GamerGirlAttackSymbol(float x, float y, Image image, GamerGirlAttack attack, int damage, Rectangle hitBox,
								 Vector2f momentum, Vector2f force, BlockLevel blockLevel) {
		this.x = x;
		this.y = y;
		this.image = image;
		this.attack = attack;
	...
	public void update(int delta, Character other){
		...
		if(!hasHit){
			...
			if(other.getHitbox().intersects(hitBox)){
				impactTime = progress;
				hasHit = true;
				other.takeDamage(damage, force, blockLevel);
