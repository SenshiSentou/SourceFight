...
public class Menu {
			...
			if(input.isKeyPressed(Input.KEY_ESCAPE)){
				...
				for(int i=0; i<2; i++){
					if(cursorPosCur[i] != null && !cursorLocked[i]){
						if(input.isKeyPressed(playerKeys[i][0])){
							if(cursorPosCur[i].y > cursorPosMin.y){
								cursorPosCur[i].y--;
								cursorPosUpdated(i);
							}
							else if(seamless){
								cursorPosCur[i].y = cursorPosMax.y;
								cursorPosUpdated(i);
							}
						}
						else if(input.isKeyPressed(playerKeys[i][1])){
							
							if(cursorPosCur[i].y < cursorPosMax.y){
								cursorPosCur[i].y++;
								cursorPosUpdated(i);
							}
							else if(seamless){
								cursorPosCur[i].y = cursorPosMin.y;
								cursorPosUpdated(i);
							}
						}
						else if(input.isKeyPressed(playerKeys[i][2])){
							if(inverted){
								if(cursorPosCur[i].x < cursorPosMax.x){
	...
	//								System.out.println("flag 3");
									cursorPosCur[i].x = cursorPosMin.x;
									cursorPosUpdated(i);
								}
							}
							else{
								if(cursorPosCur[i].x > cursorPosMin.x){
									cursorPosCur[i].x--;
									cursorPosUpdated(i);
								}
								else if(seamless){
									cursorPosCur[i].x = cursorPosMax.x;
									cursorPosUpdated(i);
								}
							}
						}
						else if(input.isKeyPressed(playerKeys[i][3])){
