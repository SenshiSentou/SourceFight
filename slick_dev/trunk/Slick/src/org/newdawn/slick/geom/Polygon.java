...
public class Polygon extends Shape {
    ...
    public Polygon(float points[]) {
        ...
        findCenter();
//        center = new float[]{(maxX + minX) * 0.5f, (maxY + minY) * 0.5f};
        calculateRadius();
        pointsDirty = true;
    }
    /**
...
//    	This is empty since a polygon must have it's points all the time.
	...
	/**
     * Get a flipped copy of the original
     * 
     * @param flipHorizontal
     * @param flipVertical
     * @return A copy of the original shape mirrored around it's center
     */
    public Polygon getFlippedCopy(boolean flipHorizontal, boolean flipVertical) {
    	checkPoints();
    	
    	Polygon poly = copy();
    	for(int i=0; i<poly.points.length; i++){
    		if(i%2 == 0 && flipHorizontal){
    			poly.points[i] = 2*getCenterX() - poly.points[i];  
    		}
    		else if(i%2 == 1 && flipVertical){
    			poly.points[i] = 2*getCenterY() - poly.points[i];
    		}
    	}
    	poly.pointsDirty = true;
    	poly.checkPoints();
